...
import static symlab.CloudAR.definition.Constants.scale;

/**
 * Created by wzhangal on 4/24/2017.
 */

public class MatchingTaskORB implements MatchingTask{
    ...
    private Mat GrayScaled = new Mat(previewHeight / scale, previewWidth / scale, CvType.CV_8UC1);

    public MatchingTaskORB(Context context) {
        ...
        for(int m = 0; m < images.size(); m++) {
            ...
            if (good_matches.size() >= 4) {
                ...
                for (int i = 0; i < good_matches.size(); i++) {
                    objList.addLast(localKeypoints.get(m).toList().get(good_matches.get(i).trainIdx).pt);
                ...
                int height = images.get(m).height();
                int width = images.get(m).width();
                ...
                String Name = "localImage" + m;

                markerGroup.addMarker(new Marker(m, Name, new Size(width, height), sceneCorners));
            } else {
